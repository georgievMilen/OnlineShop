...
router.get("/", async (req, res) => {
  ...
  if (from && to) {
    const products = await Product.find({ price: { $gte: from, $lte: to } });
    res.send(products);
  }
  if (limiter) {
  ...
  if (limiterDisc) {
    const products = await Product.find({ priceWithDiscount: { $ne: null } })
      .sort({ dateAdded: -1 })
      .limit(limiterDisc);
    res.send(products);
  }
  if (lowestPrice) {
    const products = await Product.find()
      .sort({ price: 1, priceWithDiscount: 1 })
      .limit(lowestPrice);
    res.send(products);
  } else {
...
router.delete("/:id", async (req, res) => {
  ...
  if (!product)
    return res.status(404).send("No product with the given id found.");
